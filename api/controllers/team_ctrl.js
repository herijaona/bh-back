    ...
    try {
        ...
        if (usrOwnerMail.length > 0) {
            ...
            if (usrOwnerMailI.length > 0) {
                ...
                if (yu) {
                    let m = await tools_service.mailInvitations(yu, userData, userACC);
...
module.exports.changeAdmRole = async (req, res) => {
    ...
    try {
        ...
        if (reqData.value == true) {
            upAcc = await Account.findByIdAndUpdate(acc_id, {
                $push: updateData
            }, {
                new: true
            });
...
module.exports.deleteUserFromTeam = async (req, res) => {
    ...
    let acc_id = req.ACC._id;
    try {
        let admin_defaults = await User.findById(reqData.usr_id, "admin_defaults");
        if (admin_defaults) {
            if ('admin_defaults' in admin_defaults) {
                if (admin_defaults['admin_defaults'] && reqData.reg == "adm") {
                    return sendJSONresponse(res, 200, {
        ...
        let upAcc = await Account.findByIdAndUpdate(acc_id, {
            $pull: {
